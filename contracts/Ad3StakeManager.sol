...
contract Ad3StakeManager is IAd3StakeManager, ReentrancyGuard
{
    ...
    function updateRange(IncentiveKey memory key, int24 tickLower, int24 tickUpper)
    ...
    function stakes(bytes32 incentiveId, uint256 tokenId)
        ...
        view
        override
        returns (address owner, uint160 secondsPerLiquidityInsideInitialX128, uint128 liquidity)
    {
    ...
    function _stakeToken(IncentiveKey memory key, uint256 tokenId, address from) internal {
        ...
        deposit.numberOfStakes = uint96(SafeMath.add(deposit.numberOfStakes, 1));
        incentive.numberOfStakes = uint96(SafeMath.add(incentive.numberOfStakes, 1));
        ...
        nonReentrant
    {
        ...
        {
            deposits[tokenId].numberOfStakes = uint96(SafeMath.sub(deposits[tokenId].numberOfStakes, 1));
            incentive.numberOfStakes = uint96(SafeMath.sub(incentive.numberOfStakes, 1));
            incentive.totalSecondsClaimedX128 = uint160(
                    SafeMath.add(incentive.totalSecondsClaimedX128,
                                secondsInsideX128));
    ...
    function withdrawToken(uint256 tokenId, address to)
        external
        override
    {
        require(deposits[tokenId].owner == msg.sender, 'only owner can withdraw token');
        require(deposits[tokenId].numberOfStakes == 0, 'nonzero number of stakes');
    ...
    function claimReward(address rewardToken, address to, uint256 amountRequested)
        external
        override
        nonReentrant
    {
        ...
        rewards[rewardToken][msg.sender] = rewards[rewardToken][to].sub(amountRequested);
